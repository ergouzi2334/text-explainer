...
单词或短语：
- 格式：**单词** [词性] 中文释义
- 词性标注：名词[n.] 动词[v.] 形容词[adj.] 副词[adv.] 介词[prep.] 连词[conj.] 代词[pron.]
- 中文释义简洁准确，只列最常用的1-2个义项
...
// ======== 多词批处理 Prompt ========
const BATCH_PROMPT = `你是一个词典专家。用户选中了多个英文单词（用顿号分隔），请逐一解释每个单词。

要求：
- 每个单词单独一行，格式：**单词** [词性] — 中文释义
- 释义简洁准确，只列最常用的1-2个义项
- 如果是名词标注[n.]，动词标注[v.]，形容词标注[adj.]，副词标注[adv.]
- 不需要例句，不需要展开说明
- 不要前缀后缀`;
  ...
  try {
    const resp = await fetch(DEEPSEEK_URL, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.5,
        max_tokens: isBatch ? 400 : (isTranslate || isWord) ? 200 : 500,
      }),
